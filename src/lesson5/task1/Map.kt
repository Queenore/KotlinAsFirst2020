...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((key, value) in grades) {
        if (result.containsKey(value)) result[value]?.add(key)
...
fun whoAreInBoth(a: List<String>, b: List<String>): List<String> {
    ...
    for (first in a)
        for (second in b)
            if (first == second && !map.containsKey(first)) map[first] = 1
    for ((key) in map) result.add(key)
    return result
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((key, value) in stockPrices) {
        ...
        count[key] = count.getOrDefault(key, 0.0) + 1.0
    }
    for ((key) in result) result[key] = result[key]!! / count[key]!!
    return result
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for ((key, value) in stuff) {
        if (value.first == kind && count == 0) {
            count++
            result = key
            min = value.second
        } else if (value.first == kind && value.second < min) {
            result = key
            min = value.second
        }
    }
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (elem in list)
        result[elem] = result.getOrDefault(elem, 0) + 1
    list.forEach { if (result[it] == 1) result.remove(it) }
    return result
}
...
fun hasAnagrams(words: List<String>): Boolean {
    ...
    for (elem in words) {
        set.add(elem.toSortedSet().toString())
    }
    if (words.size != set.size) return true
    return false
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for ((key, value) in map) {
        if (value.second >= 2 && map.contains(number - key))
